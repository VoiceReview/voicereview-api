...
DOCKER_CONTAINER_NAME="user_database"
...
for migration_file in "$MIGRATION_DIR"/*.sql; do
    ...
    migration_checksum=$(docker exec "$DOCKER_CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT checksum FROM private.migrations WHERE name = '$migration_file'" -t | tr -d '[:space:]')
    ...
    migration_output=$(docker exec -i "$DOCKER_CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" < "$migration_file" 2>&1)
    ...
    docker exec "$DOCKER_CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "INSERT INTO private.migrations (name, checksum) VALUES ('$migration_file', '$migration_file_checksum')"
done
